...
fignum = 150; % if fignum = 0 make no figures
...
rotVertDive = [30,20]; %***
% rotVertDive = [0:5:55]';
%     rot2 = 20*ones(1,length(rotVertDive));
%      rotVertDive= [rotVertDive,rot2'];
...
fnParms = fullfile(saveDir,sprintf('BinModel_Parms_%s.txt',...
    species));
fid = fopen(fnParms,'a');
fprintf(fid,'\r\n Num Iterations = %10d',N);
% Parameters that do not change in the loop
fprintf(fid,'\r\n %4d %4d %4d %4d %4d %4d %4d %4d',...
    diveDepth,diveDepths,SLm,SLs);
fprintf(fid,'\r\n %4d %4d %4d %4d %4d %4d %4d %4d',...
            ...
            for itParms3 = 1: srotVertForage(1)
                ...
                for itParms4 = 1: srotVertDive(1)
                    ...
                    for itr_n = 1:n  % number of simulations loop
                        ...
                        RLforHist(itr_n,:) = hist(RL_keep,RLbins);
...
%                         binnedCountsTot(itr_n,:) = binTot';
                    ...
                    lRL = log10(mRL(iRL));
                    isim = isim + 1;
                    dif(isim,itSite) = sum((lnbper(inot) - lRL(inot)).^2);
                    ...
                    fprintf(fid,'%4d %4d %4d %4d dif = %8.4f',...
                        rotVertDivem,rotVertDives,dif(isim,itSite));
                    ...
                    %
                    if fignum > 0
                        save(fullfile(saveDir,sprintf('%s_BinModelFinal_%dItr_%s.mat',...
                        ...
                        %
                         save(fullfile(saveDir,sprintf('BinModel_%dItr_%s.mat',...
                           itr_n,species)),'-mat');
                    end
                end
            end
...
if (sdiveDepth  > 1)
    ...
    ylabel('P(det)')
    saveas(gca,fullfile(saveDir,[species,'_diveDepthBin.fig']));
    save(fullfile(saveDir,sprintf('diveDepthBinModOut_%dItrs_%s.mat',...
                      itr_n,species)),'-mat');      
end
if (srotHoriz  > 1)
    figure(fignum); fignum = fignum +1; clf;
    subplot(2,1,1)
    plot(rotHoriz(:,1),dif3,'b--o')
    xlabel('rotHoriz deg'); ylabel('Error');
    subplot(2,1,2)
    for it = 1: length(siteVec)
        plot(rotHoriz(:,1),pD(:,it));
        hold on
    end
    ylabel('P(det)')
    saveas(gca,fullfile(saveDir,[species,'_rotHoriz.fig']));
    save(fullfile(saveDir,sprintf('rotHorizBinModOut_%dItrs_%s.mat',...
                      itr_n,species)),'-mat');      
end
if (srotVertForage  > 1)
    figure(fignum); fignum = fignum +1; clf; 
    subplot(2,1,1)
    plot(rotVertForage(:,1),dif3,'b--o')
    xlabel('rotVertForage deg'); ylabel('Error');
    ylabel('P(det)')
    subplot(2,1,2)
    for it = 1: length(siteVec)
        plot(rotVertForage(:,1),pD(:,it));
        hold on
    end
    saveas(gca,fullfile(saveDir,[species,'_rotVertF.fig']));
    save(fullfile(saveDir,sprintf('rotVertFBinModOut_%dItrs_%s.mat',...
                      itr_n,species)),'-mat');  
end
if (srotVertDive  > 1)
    figure(fignum); fignum = fignum +1; clf; 
    subplot(2,1,1)
    plot(rotVertDive(:,1),dif3,'b--o')
    xlabel('rotVertDive deg'); ylabel('Error');
    %ylim ([min(dif3),max(dif3)])
    subplot(2,1,2)
    for it = 1: length(siteVec)
        plot(rotVertDive(:,1),pD(:,it));
        hold on
    end
    ylabel('P(det)')
    saveas(gca,fullfile(saveDir,[species,'_rotVertD.fig']));
    save(fullfile(saveDir,sprintf('rotVertDBinModOut_%dItrs_%s.mat',...
                      itr_n,species)),'-mat');      
end         
fclose(fid);
toc