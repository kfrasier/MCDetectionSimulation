        ...
        for itParms2 = 1: sdescAngle(1)
            ...
            for itParms3 = 1: sdirect(1)
                ...
                for itParms4 = 1: sbotAngle(1)
                    ...
                    %  save(fullfile(saveDir,sprintf('%s_clickModel_%dItr_%s.mat',...
                    ...
                    if (strcmp(species,'kogia'))
                        %JAH *** remove 12 for other than Kogia!
                        RLshift = RLbins+1-12; %12 is due to 320 versus 200
                    else
                        RLshift = RLbins+1;
                    end
                    ...
                    lRL = log10(mRL(iRL));
                    isim = isim + 1;
                    diff(isim,itSite) = sum((lnper(inot) - lRL(inot)).^2);
                    ...
                    fprintf(fid,'%4d %4d %4d %4d %4d %4d %4d %4d %7.3f %7.3f diff = %8.4f',...
                        directm,minAmpSm,minAmpBm,botAngles,descentPm,...
                        diff(isim,itSite));
                    ...
                    if fignum > 0
                        ...
                        %note -12 is to correct for 100kHz data versus fullband
                        figure(fignum); fignum = fignum +1; clf;
                        hb4 = bar(RLbins+1-12,mRL,1,...
                            'barwidth', 1, 'basevalue', 1);
                        set(hb4,'EdgeColor','k','FaceColor','w')
                        xlim([thresh-13,172-12])
                        ylim([.001,50])
                        set(gca,'YScale','log')
                        xlabel(gca,'RL (dB_p_p re 1\muPa)','FontSize',16)
                        ylabel(gca, 'Percent of detections','FontSize',16)
